...
\begin{center}
	...
	\captionof{figure}{Test Using the best angles of each bit-string in different ones}
...
\begin{center}
	...
	\captionof{figure}{Results for Grover with 2 Qubits (default rotation)}
...
\begin{center}
	...
	\captionof{figure}{Results for Grover with 2 Qubits (modified rotation)}
...
%FIX: cut the image a little bit on the left sie
%FIND A WAY TO PUT THESE TWO IMAGES ON THE SIDES OF THE PAGE


	\begin{center}
		\centering
		\includegraphics[scale=0.3]{less_than.png}
		\captionof{figure}{Sets Difference Algorithm Example}
...
\begin{center}
	\includegraphics[scale=0.4]{less_than_unitary.png}
	\captionof{figure}{Unitary After applying the Sets Difference algorithm}
	\label{fig:less-than-circuit-unitary}
\end{center}

After Overlapping the phases  \ref{fig:less-than-circuit-unitary}, only the values $\{100, 101\}$ keep the $-1$ value. This way, we can do the set operation just applying some oracles with the values.
